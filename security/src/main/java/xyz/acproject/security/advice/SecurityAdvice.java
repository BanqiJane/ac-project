package xyz.acproject.security.advice;
...
public class SecurityAdvice {
        ...
        public Response handleBadCredentialsException(InternalAuthenticationServiceException e) {
            LOGGER.error("账号或密码错误:{}", e.getMessage());
            return new Response().custom(HttpCodeEnum.passworderror);
        }
    ...
    public Response handleRequestRejectedException(RequestRejectedException e){
        LOGGER.error("url安全错误:{}", e.getMessage());
        return new Response().custom(HttpCodeEnum.urlerror);
    }
    ...
    @ResponseStatus(HttpStatus.FORBIDDEN)
    public Response handleExpiredJwtException(AccessDeniedException e){
